...
const EditProfile = ({ navigation }) => {
    ...
    const editProfile = async() => {
        try{
            const response = await trackerApi.post('/cook/editProfile', {name,email,password,phone})
        ...
        <View style={{flex:1,backgroundColor:'white'}}>
            <View style={{height:280,backgroundColor:'rgb(20, 25, 43)',opacity:2}}>
                <View style={{marginLeft:20,marginTop:20,flexDirection:'row',alignItems:'center'}}>
                    <TouchableOpacity style={{flex:1}} activeOpacity={0.8} onPress={() => navigation.navigate('Profile')}>
                ...
                <Image source={{uri:profile.image}} style={{width:100,height:100,borderRadius:60,marginTop:40,alignSelf:'center'}}/>
            </View>
